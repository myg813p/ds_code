        ...
        if ex_ip != '121.133.167.1622':
            ...
            while True:
                try:
                    ...
                    driver.find_element(By.NAME, 'managerId').send_keys('dltmdwn')
                    time.sleep(0.5)
                    ...
                    url_add = 'http://gdadmin.edftr76860385.godomall.com/order/order_list_all.php?detailSearch=y&scmFl=1'
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
                ...
                today = datetime.now()
                month6 = today - timedelta(days=30)  # 6 months ago
                
                for time6 in range(6):
                    print(f'for_6_{time6+1}번째')
                    
                    today_str = today.strftime('%Y-%m-%d')
                    month6_str = month6.strftime('%Y-%m-%d')
                    
                    print(f"Today: {today_str}")
                    print(f"Month6: {month6_str}")
                    
                    # Update the dates for the next iteration
                    today = month6 - timedelta(days=1)  # Set today to the day before month6
                    month6 -= timedelta(days=30)
                        
                    go_url = url_add + \
                        f'''&key=o.orderNo&keyword=&treatDateFl=og.regDt&treatDate[]={month6_str}&treatTime[]=00:00:00&treatDate[]={today_str}&treatTime[]=23:59:59&orderTypeFl[]=&orderChannelFl[]=&orderStatus[]=o1&orderStatus[]=p1&orderStatus[]=g1&orderStatus[]=d1&orderStatus[]=d2&orderStatus[]=s1&settleKind[]=&invoiceCompanySno=0&invoiceNoFl=&withdrawnMembersOrderFl=y&orderMemoCd=&memFl=&settlePrice[]=&settlePrice[]=&receiptFl=&overDepositDay=&underDeliveryDay=&sort=og.orderNo+desc&pageNum={data_EA}&view=orderGoods&searchFl=y&applyPath=%2Forder%2Forder_list_all.php%3Fview%3DorderGoods&withdrawnMembersOrderChk=y'''
                    # print(url_add)
                    # time.sleep(10000)
                    ...
                    replace_words = ['무료배송', '의자BEST', '땡처리특가', '가격인하특가', 'MD추천', '무료배송 특가', '/낱개선택', '/무료배송','제주도배송','동서특가', '설연휴특가', '원데이특가', '원데이행사', '브랜드위크', '동서가구 단독/쿠폰다운', '주말특가', '주말 특가', '위클리딜', '식탁생활', '최저가 도전', '1% 추가할인', '사은품증정', '소파마켓', '[단독쿠폰(~4/30까지)]', 'BEST인기식탁', '[', '지정배송서비스', '★리퍼특가★']
                    ...
                    for trs in tr:
                        ...
                        if ck_code1 == False and ck_code2 == False:
                            ...
                            td = trs.find_all('td')
                            for tds in td:
                                if tds.text.count('0원') > 0 and tds.text.count('0원/')==0 and '사이즈' not in tds.text:
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
                ...
                except_cpy_except = ['온우리가구', '주식회사럼블', '에어프라이어 선반']
                for i in range(len(lists)):
                    except_cnt = 0
                    for j in range(len(except_cpy)):
                        if lists[i][1].count(except_cpy[j]) + lists[i][3].count(except_cpy[j]) > 0 and lists[i][1].count(except_cpy_except[0]) == 0 and lists[i][1].count(except_cpy_except[1]) == 0 and lists[i][3].count(except_cpy_except[2]) == 0:
                            except_cnt += 1
                    if except_cnt > 0:
